  ...
  switch (message.action) {
    ...
    case "insertLinkIntoCompose": {
      ...
      if (details.isPlainText) {
        ...
        // Build template similar to Thunderbird's CloudFile template
        let metaLines = "";
        if (fileSize) metaLines += `Size: ${fileSize}<br>`;
        metaLines += `Link: <a href="${link}" style="color:#4a90d9;">${link}</a>`;
  ...
  <div style="background:#f0f6ff;border-radius:8px;padding:12px 16px;margin-bottom:4px;">
    <table style="width:100%;border-collapse:collapse;"><tr>
      ...
      <td style="width:60px;vertical-align:top;text-align:right;">
        ...
        const sigIndex = body.indexOf('<div class="moz-signature"');
        if (sigIndex !== -1) {
          body = body.substring(0, sigIndex) + linkHtml + body.substring(sigIndex);
        } else {
          body = body.replace(/<\/body>/i, `${linkHtml}</body>`);
        }
