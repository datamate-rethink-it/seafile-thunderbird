...
class SeafileAPI {
  ...
  _extractErrorMessage(status, text, fallback) {
    if (status === 443) {
      return browser.i18n.getMessage("errorQuotaExceeded") || "Storage quota exceeded.";
    }
    if (status === 442) {
  ...
  async renameFile(server, token, repoId, path, newName) {
    ...
    if (!resp.ok) {
      ...
      throw new Error(this._extractErrorMessage(resp.status, text, "Failed to rename file"));
    }
  }
